...
if ($existing) { $imagePath = $existing['image_path'] ?: null; $pdfPath = $existing['pdf_path'] ?: null; }

if (!empty($_FILES['image']['name'])) {
  $ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
  $allowed = ['jpg','jpeg','png','webp'];
  if (!in_array($ext, $allowed, true)) {
    header("Location: " . BASE_URL . "/admin_tour_form.php?id={$id}&error=Foto+jo+valide"); exit;
  }
...
if (!empty($_FILES['pdf']['name'])) {
  ...
  if ($ext !== 'pdf') {
    header("Location: " . BASE_URL . "/admin_tour_form.php?id={$id}&error=PDF+jo+valid"); exit;
  }
